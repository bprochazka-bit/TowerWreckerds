...
reference track and (optionally) a performer, and the model writes a fresh brief
— new lyrics, mood, tempo, and style cues — that reinterprets the original in
that performer's voice. The cover renders through ACE-Step like any other track,
and the source reference is recorded on the track page for provenance.

## Cover art (sd.cpp)

Album covers are generated by **[stable-diffusion.cpp](https://github.com/leejet/stable-diffusion.cpp)**.
In **Admin → sd.cpp (cover art)** set the path to the `sd` executable
(`sdcpp_path`) and the model weights (`sdcpp_model`), plus steps, output size,
and CFG scale. The app shells out to:

```
<sdcpp_path> -m <model> -p "<prompt>" -o cover.png --steps N -W S -H S --cfg-scale C -s SEED
```

Open any album and click **Generate cover**; the prompt is built from the
album's title, owner, genre, concept, and style tags. With **Mock mode** on (or
no path configured) a deterministic placeholder image is produced instead, so
the rest of the flow still works with no backend.
